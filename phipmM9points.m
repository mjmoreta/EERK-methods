...
%
%         w = phi_0(tM^{-1}A) u(:, 1) + t phi_1(tM^{-1}A) u(:, 2) + 
%             t^2 phi_2(tM^{-1}A) u(:, 3) + ...  
%
...
%         symmetric. In this case, A is symmetric
%
% PARAMETERS:
%   t        - constant value represent time.
%   A        - the matrix argument of the phi functions.
%   A11, A12 - the matrices that form matrix A
%   J        - A11 and A12 are (J-1)x(J-1) matrices
%   M    - The mass matrix
...
%   symm - true if the matrix A is symmetric. In this case, A is always
%   symmetric
...
% We dont use matrix A. We use the matrices that form matrix A. They are
% smaller that matrix A.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%nnze = nnz(A)
A11=(-10/3)*diag([ones(J-1,1)])+(2/3)*(diag(ones(J-2,1),1)+diag(ones(J-2,1),-1));
A12=(2/3)*diag([ones(J-1,1)])+(1/6)*(diag(ones(J-2,1),1)+diag(ones(J-2,1),-1));
nnze=(J-1)*nnz(A11)+2*(J-2)*nnz(A12);
h=1/J;
mult=12/h^2;
...
if nargin < 6
  m = 10;
  if nargin < 5
    if max(full(max(A11-A11')))+max(full(max(A12-A12'))) < 100*eps
      symm = true;
    else
      symm = false;      
    end   
...
% Infinity norms
Anorm = max(norm(A11, 'inf')+2*norm(A12, 'inf'),norm(A11, 'inf')+norm(A12, 'inf'));
vnorm = norm(u(:, 1), 'inf');
...
while tnow < tout
  ...
  if j == 0
    ...
    for i = 1:p-1
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      vecu=multA(J-1,int(:,i));
     
      int(:, i+1) = mult*multMm1(J-1,vecu)+up(:, i+1);
      
      %int(:, i+1) = M\A*int(:, i)+up(:, i+1);
            
    end 

    % Normalize initial vector. We use nomrM, defined as sqrt(u'Mu)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    beta = norm(int(:, end));
    %beta = sqrt(int(:, end)'*M*int(:, end));
    if beta == 0
