...
for ll=1:8
    ...
    for r=1:1
        ...
        % u(t,0)=cos(t), u(t,1)=cos(t+1),
        
        vecind=zeros(dim,1);
        u0=cos(t);
        u1=cos(t+1);
        vecind(1)=8*hdiv*u0/3;
        vecind(2)=-hdiv*u0/3;
        vecind(dim-1)=-hdiv*u1/3;
        vecind(dim)=8*hdiv*u1/3;         
        vecind=sparse(vecind);
        vecbound=M\vecind;        
        
        vecb2(:,2)=vecb2(:,2)+vecbound;
        ...
        for ii=1:dim
            ...
            vecb(ii,2)=U1(ii)^2+funh2+vecbound(ii);
        end    
        ...
        vecind(1)=-fut0/15;
        vecind(2)=fut0/30;
        vecind(dim-1)=fut1/30;
        vecind(dim)=-fut1/15;        
        vecind=sparse(vecind);        
        vecbound=M\vecind;             
               
        vecb(:,2)=vecb(:,2)+vecbound;
